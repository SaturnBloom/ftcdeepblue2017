...
@TeleOp(name="BHA: TeleOp (with Lift)", group="BHA")
public class Lift_TeleOp_Test extends OpMode{

    /* Declare OpMode members. */
    HardwareBHA_Test robot       = new HardwareBHA_Test(); //define a robot object from HardwareBHA_Test class

    /* Declare Motor Power constants */
    //final double BALL_COLLECTOR_POWER = -1.0; // 100q% Power
    //final double BALL_LIFT_POWER = 0.8; // 80% Power
    //final double BALL_SHOOTER_POWER = 0.9; // 80% Power
    ...
    public void init() {
        ...
        telemetry.addData("Say", "HardwareBHA_Test Initialized.");    //
    }
    ...
    public void loop() {
        ...
        if (gamepad1.a && up == false) {
            robot.ballLift.setPower(1);
            up = true;
        } else if (gamepad1.a && up == true) {
            robot.ballLift.setPower(-1);
            up = false;
        }
        if (gamepad1.b) {
